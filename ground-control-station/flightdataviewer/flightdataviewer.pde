...
//ArrayList<Float[]> parsedFlightData = new ArrayList<>();

ArrayList<float[]> parsedFlightData = new ArrayList<>();

byte thisByte;

boolean useFlightData = true;
int Iteration = 0;
...
void setup()
{
  ...
  /*String[] flightDatas = loadStrings("flightdata3.csv");
  for (String flightData : flightDatas) {
      String[] data = split(flightData, ",");
  ...
  while (index < bytes.length){
    if (bytes[index] == MSG_START){
      msg_type = bytes[++index];
      msg_size = bytes[++index];
      data_index = index;
      
      float[] floatData = new float[msg_size];
      
       for (int i = 0; i < msg_size/4 && i*4 < bytes.length; i++){
        floatData[i] = ByteBuffer.wrap(bytes, data_index+i*4, 4).order(ByteOrder.LITTLE_ENDIAN).getFloat();
        }
        
        parsedFlightData.add(floatData);
    }
